...
function [C,F,E] = FD_gv(X,N,K_c,extra_maximum,L2_initpoints,display)
%FD_gv   Compute the group-velocity optimal coefficients
%   INPUT:
%    FD_gv(X,N,K_c) returns the finite-difference coefficients to
%    get the N-th derivative at x=0 from samples at X, accurate up to a
%    wave-number K_c, with standard number of maxima.
%
%    FD_gv(X,N,K_c,extra_maximum) expects 'extra_maximum' more
%    maxima. For extra_maxima>1, this results in an L2 inversion.
%
%    FD_gv(X,N,K_c,extra_maximum,L2_initpoints) Initializes with
...
%    L2_initpoints < 0, i.e., -1. E.g., FD_gv([-5.2:3.8],1,1,0,-1)
%
%    FD_gv(X,N,K_c,extra_maximum,L2_initpoints,display) allows a 
...
%   EXAMPLES:
%    ASYMMETRIC
%     FD_gv([-3.2:0.8],0,1,0,10)
%     FD_gv([-3.2:0.8]+randn(1,5)*0.2,0,1,0,10)
%     FD_gv([-10.5:10.5]+0.2,0,2.0,0,1e4)
%     X=[-3.2:3.6]+randn(1,7)*0.5;FD_gv(X,3,1,0,500)
%     FD_gv([-10.5:10.5]+0.2,0,1.5)
%    SYMMETRIC
%     FD_gv([-4.5:4.5],1,1.9,1,1e4)
%     FD_gv([-10.5:10.5],2,1.9,0,1e4)
%     FD_gv([-10:10],2,1.9,1,1e4)
%
...
if EM<=1 && L2>0
    ...
    C = FD_gv(X,N,K_c,L2,[],false)'; % Run L2-inversion
    intp = Erf(ks,C);
