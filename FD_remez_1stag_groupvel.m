...
function [D] = FD_remez_1stag_groupvel(L,grerr)
%FD_REMEZ_1STAG_GROUPVEL   Compute staggered grid optimal first-derivative symmetric coefficients
%   INPUT
%    FD_REMEZ_1STAG_GROUPVEL(L,grerr) computes the coefficients for
%    half-order L, for a first order derivative with groupvellute cost-function
%  
%   OUTPUT
%    D = L^inf optimal coefficients for half-order scheme
%
%   EXAMPLES
%    FD_remez_1stag_groupvel(40,1e-5)
%
...
ks = linspace(1-7,pi-1e-7,L)';

% --- Dense grid for interpolation
k=linspace(0,pi,L*200);

% --- Matrices D(=LHS) and d(=RHS), eq. (21)
LHS = @(L,E,KS) ([1:2:2*L-1] .* cos( KS/2 * [1:2:2*L-1] ));
RHS = @(L,E,KS) 1 + (-1).^([1:L]'+L)*E;
  
% --- Relative error: eq. (15)
error = @(d,k) 2.*([1:L+1/2]-1/2).*cos( k*([1:L+1/2]-1/2) ) * d -1;
